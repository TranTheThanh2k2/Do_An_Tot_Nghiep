...
import { useGetAllDoctorsQuery, useGetDoctorByIdQuery } from "../../Redux/Doctor/api"; // Import API slice
import doctor1 from "../../img/doctor1.png"; // Placeholder ảnh bác sĩ
...
const CreateAppoiment = () => {
 ...
 const Navigate = useNavigate();
  ...
  // Hiển thị spinner khi đang tải danh sách bác sĩ
  if (isLoading) return <Spin size="large" />;

  // Hiển thị thông báo lỗi nếu không thể tải danh sách bác sĩ
  if (error) return <p>Đã xảy ra lỗi khi lấy danh sách bác sĩ!</p>;

  // Tạo danh sách chuyên khoa từ dữ liệu bác sĩ
  const specialties = Array.from(new Set(doctors?.doctors?.map((doctor) => doctor.specialty)));
  ...
  const handleViewDetails = (doctorId) => {
    setSelectedDoctorId(doctorId); // Lưu doctorId để gọi API lấy chi tiết bác sĩ
  };

  // Hàm đóng Modal và reset lại doctorId
  const handleCloseModal = () => {
    setSelectedDoctorId(null); // Reset doctorId khi đóng modal
  };
        ...
        <div className="flex flex-col md:flex-row">
              ...
              {specialties.map((specialty, index) => (
                <li
                  ...
                  className={`text-lg cursor-pointer ${
                    selectedSpecialty === specialty ? "text-blue-600 font-bold" : ""
                  }`}
                >
                  {specialty}
                </li>
          ...
          <div className="w-full md:w-3/4 p-4">
            {filteredDoctors.length === 0 ? (
              <p>Không có bác sĩ nào phù hợp với chuyên khoa đã chọn.</p>
            ) : (
              filteredDoctors?.map((doctor) => (
                <Card key={doctor._id} className="mb-4 p-4">
                  <div className="flex flex-col md:flex-row">
                    <div className="md:w-1/3 mb-4 md:mb-0">
                      <img src={doctor1} alt="Doctor" className="w-full rounded-md" />
                    </div>
                    <div className="md:w-2/3 md:ml-6">
                      <h3 className="text-2xl font-bold text-blue-600">PGS.TS.BS {doctor?.user?.fullName}</h3>
                      <p className="text-lg">
                      ...
                      <div className="mt-4 flex justify-center space-x-4">
                        <Button type="primary" onClick={() => handleViewDetails(doctor._id)}>
                          Xem Chi Tiết
                        </Button>
                        <Button type="primary" className="bg-blue-600" onClick={() => handleBookAppointment(doctor._id)}>
                        Đặt Lịch Khám
                      </Button>
                      </div>
                    </div>
